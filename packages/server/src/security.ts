...
function isFnosGatewayHostOrigin(origin: string | undefined, host: string | undefined): boolean {
  if (String(process.env.HERMES_FNOS_MODE || '').trim() !== '1') return false
  const requestHost = normalizeHost(host)
  // fnOS may remove the public gateway port from Host while preserving it in
  // the browser Origin. Only allow this narrow same-host case in fnOS mode.
  if (!origin || !requestHost || requestHost.includes(':')) return false
  try {
    const parsed = new URL(origin)
...
export function shouldRejectUpgradeOrigin(req: IncomingMessage, corsOrigins = ''): boolean {
  ...
  return !isOriginAllowed(selectedOrigin, req.headers.host, corsOrigins)
    && !isFnosGatewayHostOrigin(selectedOrigin, req.headers.host)
...
export function securityHeaders(options: { allowSameOriginFrames?: boolean } = {}): Middleware {
  return async (ctx, next) => {
    ctx.set('X-Content-Type-Options', 'nosniff')
    ctx.set('X-Frame-Options', options.allowSameOriginFrames ? 'SAMEORIGIN' : 'DENY')
    ...
    ctx.set('Content-Security-Policy', [
      ...
      "base-uri 'self'",
      "object-src 'none'",
      options.allowSameOriginFrames ? "frame-ancestors 'self'" : "frame-ancestors 'none'",

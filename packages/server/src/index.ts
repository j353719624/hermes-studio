...
import { createFnosFeatureGuard, createFnosGatewayMiddleware } from './middleware/fnos-gateway'
import { getLoopbackBaseUrl, listenForConfig } from './services/server-listener'
import {
...
export async function bootstrap() {
  ...
  if (config.fnos) {
    console.log('[bootstrap] desktop Coding Agent initialization skipped on fnOS')
  } else {
    try {
  ...
  app.use(createFnosGatewayMiddleware())
  app.use(securityHeaders({ allowSameOriginFrames: config.fnos }))
  ...
  app.use(createFnosFeatureGuard())
  ...
  const listenResult = await listenForConfig(app, {
    port: config.port,
    host: config.host,
  ...
  const loopbackBaseUrl = getLoopbackBaseUrl(servers, config.port)
  process.env.HERMES_WEB_UI_URL = loopbackBaseUrl

  if (config.fnos) {
    try {
      await injectBundledMcpServer()
    } catch (err) {
      logger.warn(err, '[bootstrap] failed to inject bundled MCP server on fnOS')
      console.warn('[bootstrap] failed to inject bundled MCP server on fnOS:', err instanceof Error ? err.message : err)
    }
  }
  ...
  new GroupAgentRelayServer(groupChatServer.getIO(), groupChatServer)
  ...
  servers.forEach((httpServer) => {
    httpServer.on('upgrade', (req: any, socket: any) => {
      ...
      const pathname = stripPublicBasePath(url.pathname, config.publicBasePath)
      const socketIoRequest = url.pathname === config.socketIoPath
  ...
  if (!config.fnos) startVersionCheck()
}

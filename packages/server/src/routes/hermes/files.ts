...
function isProfileConfigPath(relativePath: string): boolean {
  return relativePath.replace(/\\/g, '/').replace(/^\/+/, '') === 'config.yaml'
}

async function refreshProfileBridge(profile: string | undefined): Promise<void> {
  if (!profile) return
  try {
    const endpoint = getAgentBridgeManager().getRuntimeState().endpoint
    if (!endpoint) return
    await new AgentBridgeClient({ endpoint, timeoutMs: 5000, connectRetryMs: 0 }).destroyProfile(profile)
    logger.info('[files] refreshed Hermes Bridge profile after config.yaml write: %s', profile)
  } catch (err) {
    // Config writes must remain successful even when the optional Bridge is down.
    logger.warn(err, '[files] failed to refresh Hermes Bridge profile after config.yaml write: %s', profile)
  }
}
...
fileRoutes.put('/api/hermes/files/write', requireSuperAdmin, async (ctx) => {
  ...
  try {
    ...
    if (isProfileConfigPath(relativePath)) {
      await refreshProfileBridge(requestedProfile(ctx))
    }

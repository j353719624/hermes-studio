...
export async function fnosSession(ctx: Context) {
  if (!config.fnos || !isTrimAdminHeader(ctx.get('x-trim-isadmin'))) {
    ctx.status = 404
    ctx.body = { error: 'Not found' }
    return
  }

  const user = ensureFnosSuperAdmin(ctx.get('x-trim-username'))
  if (!user) {
    ctx.status = 403
    ctx.body = { error: 'An active Studio super administrator is required' }
    return
  }

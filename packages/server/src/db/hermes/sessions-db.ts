...
function sessionDbPath(): string {
  return joinHermesPath(getActiveProfileDir(), 'state.db')
}

function sessionDbPathForProfile(profile?: string): string {
  const name = String(profile || '').trim()
  if (!name) return sessionDbPath()
  if (name === 'default') return joinHermesPath(getHermesBaseDir(), 'state.db')
  return joinHermesPath(getHermesBaseDir(), 'profiles', name, 'state.db')
}

function joinHermesPath(base: string, ...parts: string[]): string {
  return base.startsWith('/') ? posixPath.join(base, ...parts) : join(base, ...parts)
}
...
export async function getSkillUsageStatsFromDb(
  ...
  const db = fnosMode
    ? getDb()
    : existsSync(dbPath)
      ? await openSessionDb(profile)
      : null
  if (!db) return emptySkillUsageStats(safeDays)
  try {
    const profileClause = fnosMode && profile ? ' AND s.profile = ?' : ''
    const queryParams = fnosMode && profile ? [since, since, profile] : [since, since]
    ...
    const selectCandidateRows = (predicate: string): Record<string, unknown>[] => {
      const recentRows = db.prepare(`
        ...
        WHERE m.role = 'tool'
          AND (${predicate})
          AND s.started_at > ?
          AND ${eventTimeExpression} > ?${profileClause}

...
export function findFirstActiveSuperAdmin(): UserRecord | null {
  ...
  const row = db.prepare(
    `SELECT * FROM ${USERS_TABLE} WHERE role = 'super_admin' AND status = 'active' ORDER BY id ASC LIMIT 1`,
...
export function ensureFnosSuperAdmin(username: string): UserRecord | null {
  const current = findFirstActiveSuperAdmin()
  if (current) return current
  if (countUsers() > 0) return null

  const normalized = username.trim().slice(0, 64) || 'fnos-admin'
  return createUser({
    username: normalized,

...
function hermesMcpServerConfig(profile: string, serverName: string, toolset: string): { command: string; args?: string[]; env: Record<string, string> } {
  ...
  const webUiUrl = process.env.HERMES_WEB_UI_URL?.trim() || `http://127.0.0.1:${process.env.PORT || '8648'}`
  return {
...
function parseCodexExternalMcpBlocks(...contents: Array<string | null | undefined>): string[] {
  ...
  for (const content of contents) {
    ...
    const flush = () => {
      ...
      // codex_apps is a first-party ChatGPT Apps connector. It is not a normal
      // user MCP server and cannot initialize for Hermes' custom providers
      // (for example MiniMax), so do not carry it into the isolated launch.
      if (currentServer !== 'codex_apps' && !HERMES_MCP_SERVER_NAMES.has(currentServer) && !LEGACY_HERMES_MCP_SERVER_NAMES.has(currentServer) && !isManaged) {
...
export async function prepareCodingAgentLaunch(id: string, input: CodingAgentLaunchInput): Promise<CodingAgentLaunchResult> {
  ...
  if (tool.id === 'claude-code') {
    ...
    const featureConfig = [
      '',
      '[features]',
      // The Codex Apps connector targets the ChatGPT backend. Hermes scoped
      // launches use the selected provider and should not block startup while
      // waiting for an unavailable codex_apps MCP session.
      'apps = false',

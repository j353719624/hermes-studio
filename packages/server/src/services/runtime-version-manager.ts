...
export interface RemoteVersionManifest extends StudioVersionManifest {}
...
export function runtimePlatformKey(platformName = process.platform, archName = process.arch): string {
...
export function downloadAssetUrl(assetName: string, tag: string, source: VersionDownloadSource): string {
  if (source === 'github') {
...
export function validateRuntimeDirectory(root: string, expectedPlatform: string): void {
...
export function runtimeVersionManifestUrl(): string {
  return process.env.HERMES_FNOS_RUNTIME_MANIFEST_URL?.trim()
    || process.env.HERMES_WEB_UI_VERSION_MANIFEST_URL?.trim()
...
export async function fetchRemoteVersions(): Promise<{ manifest: RemoteVersionManifest | null; error: string }> {
  const primaryUrl = runtimeVersionManifestUrl()
  try {
    return { manifest: await fetchVersionManifest(primaryUrl) as RemoteVersionManifest, error: '' }
  } catch (primaryError) {
    ...
    try {
      return { manifest: await fetchVersionManifest(FALLBACK_REMOTE_MANIFEST_URL) as RemoteVersionManifest, error: '' }
    } catch (fallbackError) {
...
export function downloadFile(url: string, target: string, onProgress?: DownloadProgressHandler, redirects = 5): Promise<void> {
...
export async function extractTarGzip(archive: string, targetRoot: string): Promise<void> {
  await tar.x({
    file: archive,
    cwd: targetRoot,
    preserveOwner: false,

...
const DEFAULT_STREAM_POLL_INTERVAL_MS = 25

function streamPollIntervalMs(): number {
  const configured = Number(process.env.HERMES_AGENT_BRIDGE_STREAM_POLL_MS || '')
...
export class AgentBridgeClient {
  ...
  async getOutput(runId: string, cursor = 0, eventCursor = 0, options: AgentBridgeRequestOptions = {}): Promise<AgentBridgeOutput> {
  ...
  async *streamOutput(
    ...
    const intervalMs = options.intervalMs ?? streamPollIntervalMs()
    let cursor = 0
    let eventCursor = 0
    for (;;) {
  ...
  async interrupt(sessionId: string, message?: string, profile?: string): Promise<AgentBridgeResponse> {
    return this.request({
      action: 'interrupt',
      session_id: sessionId,
      message,
  ...
  async steer(sessionId: string, text: string, profile?: string): Promise<AgentBridgeResponse> {
    return this.request({
      action: 'steer',
      session_id: sessionId,
      text,
  ...
  async approvalRespond(approvalId: string, choice: string): Promise<AgentBridgeResponse> {
  ...
  async destroyAll(): Promise<AgentBridgeResponse> {
    return this.request({ action: 'destroy_all' }, { serialize: true })
  }

  async destroyProfile(profile: string): Promise<AgentBridgeResponse> {
  ...
  async destroy(sessionId: string, profile?: string, workerKey?: string): Promise<AgentBridgeResponse> {
    return this.request({
      action: 'destroy',
      session_id: sessionId,
      ...(profile ? { profile } : {}),

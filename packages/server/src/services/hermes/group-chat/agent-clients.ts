...
export class AgentClient implements GroupAgentExecutor {
    ...
    async connect(port?: number): Promise<void> {
        ...
        const baseUrl = process.env.HERMES_WEB_UI_URL?.trim() || `http://127.0.0.1:${actualPort}`
        const token = await getToken()

        this.socket = io(`${baseUrl.replace(/\/$/, '')}/group-chat`, {
            path: serverConfig.socketIoPath,
            auth: {
    ...
    private async replyToMentionWithChatRun(
        ...
        if (!this.chatRunService) throw new Error('Chat run service is not ready')
        const responseRunId = groupMessageId(roomId, this.profile)
    ...
    async replyToMention(
        ...
        const runMessageId = groupMessageId(roomId, this.profile)
        let partIndex = 0
    ...
    private async recordToolCompleted(roomId: string, sessionId: string, ev: Record<string, unknown>): Promise<boolean> {
        ...
        const externalToolCallId = rawId
            ...
            || groupToolCallId(
                null,
                initialToolName,
                `${roomId}:${sessionId}:${inferResponseRunId(groupMessageId(roomId, this.profile))}`,
                this.nextAnonymousToolCallSequence(),
            )
        ...
        const runMessageId = this.pendingToolBaseIds.get(toolCallId) || groupMessagePartId(groupMessageId(roomId, this.profile), 0)
...
function groupMessageId(roomId: string, profile: string): string {
...
export class AgentClients {
    ...
    async _drainRoomQueue(roomId: string): Promise<void> {
        await this._drainRoomQueues(roomId)
    }

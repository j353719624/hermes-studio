...
import { getOrCreateGatewayApiKey } from './gateway-api-client'
...
function startGatewayRunManagedInternal(
  ...
  const child = spawnHermesWithBin(hermesBin, ['gateway', 'run', '--replace'], {
    ...
    env: {
      ...
      // Hermes Agent 0.20.4 enables its loopback API server when this key is
      // present. Keep it stable per profile so Studio and Gateway share auth
      // across supervised restarts without exposing it to the Web UI.
      API_SERVER_KEY: getOrCreateGatewayApiKey(profileDir),
    },
  })
  child.unref()

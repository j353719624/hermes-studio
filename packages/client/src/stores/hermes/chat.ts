...
export const useChatStore = defineStore('chat', () => {
  ...
  function completionNotificationAgent(session: Session): { icon: string } {
    ...
    if (codingAgentId === 'codex') {
      return { icon: getPublicAssetUrl('/coding-agents/codex-openai.png') }
    }
    if (codingAgentId === 'claude-code') {
      return { icon: getPublicAssetUrl('/coding-agents/claude-code.svg') }
    }
    if (codingAgentId === 'pi') {
      return { icon: getPublicAssetUrl('/coding-agents/pi.svg') }
    }
    if (codingAgentId === 'ekko-agent') {
      return { icon: getPublicAssetUrl('/coding-agents/ekko-agent.png') }
    }
    return { icon: getPublicAssetUrl('/coding-agents/hermes.png') }
  }

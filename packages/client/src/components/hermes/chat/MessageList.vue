...
// The gateway can briefly keep a run marked as active after the final
// assistant message has already been committed. Do not show a second
// "thinking" row underneath a completed answer during that interval.
const hasCompletedAssistantReply = computed(() => {
  ...
  return messages.slice(lastInputIdx + 1).some((message) =>
...
const displayMessages = computed(() => {
  ...
  return groupCompletedToolsByRun(mergeThinkingMessages(visibleMessages));
});
  ...
  <div class="message-list-shell">
    ...
    >
      ...
      <template #after>
        ...
        <div
          v-if="showThinkingIndicator || visibleToolCalls.length > 0 || chatStore.compressionState || chatStore.abortState"

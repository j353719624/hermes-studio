...
export async function loginWithFnos(): Promise<LoginResponse> {
  const res = await fetch(`${getBaseUrlValue()}/api/auth/fnos-session`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  })
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    const err: any = new Error(data.error || 'fnOS login failed')
    err.status = res.status
    throw err
  }

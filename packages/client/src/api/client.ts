...
const DEFAULT_BASE_URL = BUILD_BASE_PATH
const FNOS_MODE = import.meta.env.VITE_HERMES_FNOS_MODE === '1'
const ACTIVE_PROFILE_STORAGE_KEY = 'hermes_active_profile_name'

export function isFnosMode(): boolean {
  return FNOS_MODE
}
...
function splitServerBase(base: string): { origin: string; path: string } {
  ...
  try {
    ...
    return { origin: '', path: base.startsWith('/') ? base.replace(/\/$/, '') : '' }
  }
}

export function getSocketIoConnection(namespace: string): { url: string; path: string } {
  const base = splitServerBase(getBaseUrl())
  const normalizedNamespace = namespace.startsWith('/') ? namespace : `/${namespace}`
  return {
    url: `${base.origin}${normalizedNamespace}`,
    path: `${base.path}/socket.io` || '/socket.io',
  }
}
...
export function buildWebSocketUrl(path: string): string {
  ...
  const fallbackOrigin = typeof location === 'undefined' ? 'http://hermes.local' : location.origin
  const httpUrl = base
    ? new URL(`${base}${normalizedPath}`, fallbackOrigin)
    : new URL(normalizedPath, fallbackOrigin)

...
import { request, getApiKey, getSocketIoConnection } from '../client'
...
export function connectGroupChat(opts?: {
    ...
    // The global pending-actions host can create the socket before the login
    // session or invite context is ready. Reusing that instance leaves the
    // group room visible through REST while its realtime channel remains
    // permanently unauthorized. Re-key the instance by its auth context and
    // explicitly resume a disconnected socket when the context is unchanged.
    const authKey = [inviteCode, token, userId, opts?.authUserId ?? ''].join('\u0000')
    ...
    const connection = getSocketIoConnection('/group-chat')

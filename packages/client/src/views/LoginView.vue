...
import { fetchAuthStatus, loginWithFnos, loginWithPassword } from "@/api/auth";
...
const fnosMode = import.meta.env.VITE_HERMES_FNOS_MODE === "1";
...
onMounted(async () => {
  if (fnosMode) {
    loading.value = true;
    try {
      const session = await loginWithFnos();
  ...
  <div class="login-view">
    <div class="login-card">
      <div class="login-logo">
        <img :src="getPublicAssetUrl('/logo.png')" alt="Hermes" width="80" height="80" />
      </div>
      <h1 class="login-title">{{ t("login.title") }}</h1>
      <p class="login-desc">{{ t("login.description") }}</p>
      <p class="login-default-hint">{{ t("login.defaultCredentialsHint") }}</p>

      <form v-if="!fnosMode" class="login-form" @submit.prevent="handleLogin">
        <input
          v-model="username"
          type="text"
          class="login-input"
      ...
      <div v-else-if="loading" class="login-default-hint">...</div>

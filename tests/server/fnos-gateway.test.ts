...
describe('fnOS gateway identity', () => {
  ...
  it('blocks update and local STT operations while preserving cloud STT', () => {
    ...
    expect(isFnosFeatureDisabled('/api/hermes/runtime-versions')).toBe(true)
    ...
    expect(isFnosFeatureDisabled('/api/hermes/stt/local-model/download')).toBe(true)
    expect(isFnosFeatureDisabled('/api/hermes/stt/local-stream/session/chunk')).toBe(true)
    expect(isFnosFeatureDisabled('/api/hermes/stt/settings/local')).toBe(true)
    expect(isFnosFeatureDisabled('/api/hermes/stt/settings/active', { provider: 'local' })).toBe(true)
    expect(isFnosFeatureDisabled('/api/hermes/stt/settings/openai', { activeProvider: 'local' })).toBe(true)

...
describe('workspace path validation', () => {
  ...
  it('allows folders under an explicitly authorized external root', async () => {
    const externalRoot = await mkdtemp(join(tmpdir(), 'hermes-workspace-path-shared-'))
    const folder = join(externalRoot, 'project')
    await mkdir(folder)
    process.env.TRIM_DATA_ACCESSIBLE_PATHS = externalRoot
    try {
      const { resolveAllowedWorkspaceFolder } = await import('../../packages/server/src/services/hermes/workspace-path')
      await expect(resolveAllowedWorkspaceFolder(folder)).resolves.toEqual({ base: externalRoot, fullPath: folder })
      await expect(resolveAllowedWorkspaceFolder(join(externalRoot, '..'))).resolves.toBeNull()
    } finally {

...
const destroyProfileMock = vi.fn(async () => ({ ok: true }))
const agentBridgeClientMock = vi.fn(function (this: { destroyProfile: typeof destroyProfileMock }) {
  this.destroyProfile = destroyProfileMock
})
...
vi.mock('../../packages/server/src/services/hermes/agent-bridge', () => ({
  AgentBridgeClient: agentBridgeClientMock,
  getAgentBridgeManager: vi.fn(() => ({
    getRuntimeState: () => ({ endpoint: 'tcp://127.0.0.1:18765' }),
  })),
}))
...
describe('file routes path metadata', () => {
  beforeEach(() => {
    ...
    destroyProfileMock.mockReset()
    destroyProfileMock.mockResolvedValue({ ok: true })
    agentBridgeClientMock.mockClear()
  })
  ...
  it('refreshes the matching Bridge profile after writing config.yaml', async () => {
    provider.writeFile.mockResolvedValue(undefined)

    const ctx: any = {
      request: { body: { path: 'config.yaml', content: 'agent:\n  reasoning_effort: none\n' } },
      state: superAdminState(),
      body: null,
    }
    ...
    expect(agentBridgeClientMock).toHaveBeenCalledWith({
      endpoint: 'tcp://127.0.0.1:18765',
      timeoutMs: 5000,
      connectRetryMs: 0,
    })
    expect(destroyProfileMock).toHaveBeenCalledWith('research')
    expect(ctx.body).toEqual({ ok: true, path: 'config.yaml' })
  })
